...
class TestMCTS(unittest.TestCase):
    ...
    def test_initialisation(self):
        self.assertEqual(self.game.fen, 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1')
        ...
        with self.subTest('Test mosse valide all\'appertura'):
            ...
            self.assertEqual(val.possible_moves,['a2a3', 'a2a4', 'b1a3', 'b1c3', 'b2b3', 'b2b4', 'c2c3', 'c2c4', 'd2d3', 'd2d4', 'e2e3', 'e2e4', 'f2f3', 'f2f4', 'g1f3', 'g1h3', 'g2g3', 'g2g4', 'h2h3', 'h2h4']) # all possible opening moves

        with self.subTest('Test mosse fen'):
            self.game.parameters = 'd2d4' # two valid moves
            self.game.action = Actions.MOVE
            val = dispatch(self.game)

            self.game.fen = val.fen
            self.assertEqual(self.game.fen,'rnbqkbnr/pppppppp/8/8/3P4/8/PPP1PPPP/RNBQKBNR b KQkq d3 0 1')

            self.game.parameters = 'd7d5'
            self.game.action = Actions.MOVE
            val = dispatch(self.game)

            self.game.fen = val.fen
            self.assertEqual(self.game.fen, 'rnbqkbnr/ppp1pppp/8/3p4/3P4/8/PPP1PPPP/RNBQKBNR w KQkq d6 0 2')
        
        with self.subTest('Test mossa invalida (Re volante)'):
            self.game.parameters = 'e1e8'
            self.game.action = Actions.MOVE
...
class TestPiecesMoves(unittest.TestCase):
    ...
    def test_Pawn(self):
        ...
        self.game.action = Actions.LIST_MOVE
        val = dispatch(self.game)
        self.assertEqual(val.possible_moves,['a7a6', 'a7a5', 'a7b6', 'h8h7', 'h8g8', 'h8g7'])

    def test_Rook(self):
        self.game.fen = '7k/8/8/3R4/8/8/8/7K w - - 4 20'
        self.game.action = Actions.LIST_MOVE
        val = dispatch(self.game)
        self.assertEqual(val.possible_moves,['d5d1','d5d2','d5d3','d5d4','d5d6','d5d7','d5d8','d5a5','d5b5','d5c5','d5e5','d5f5','d5g5','d5h5','h1h2','h1g1','h1g2']) # full cross

        self.game.fen = '7k/8/8/1p1R4/8/3P4/8/7K w - - 4 20'
        self.game.action = Actions.LIST_MOVE
        val = dispatch(self.game)
        self.assertEqual(val.possible_moves,['d3d4','d5d4','d5d6','d5d7','d5d8','d5b5','d5c5','d5e5','d5f5','d5g5','d5h5','h1h2','h1g1','h1g2']) # cross \w enemy e ally

    def test_Knight(self):
        self.game.fen = '7k/8/8/3N4/8/8/8/7K w - - 4 20'
        self.game.action = Actions.LIST_MOVE
        val = dispatch(self.game)
        self.assertEqual(val.possible_moves,['d5b4','d5b6','d5c3','d5c7','d5f4','d5f6','d5e3','d5e7','h1h2','h1g1','h1g2'])
...
class TestSpecialSituation(unittest.TestCase): # TODO pensare ad un nome migliore
    ...
    def test_promotion(self):
        ...
        game.action = Actions.LIST_MOVE
        val = dispatch(game)
        self.assertEqual(['d7d8r', 'd7d8b', 'd7d8n', 'd7d8q', 'h1h2', 'h1g1', 'h1g2'],val.possible_moves)

        game.action = Actions.MOVE
        game.parameters = 'd7d8n'
...
class TestFogOfWar(unittest.TestCase):
    ...
    def test_openingFogOfWar(self):
        ...
        with self.subTest('Black fog of war'):
            self.assertEqual('rnbqkbnr/pppppppp/8/8/????????/????????/????????/???????? w - - 0 1',val.black_view)

        with self.subTest('White fog of war'):
            self.assertEqual('????????/????????/????????/????????/8/8/PPPPPPPP/RNBQKBNR w - - 0 1',val.white_view)
